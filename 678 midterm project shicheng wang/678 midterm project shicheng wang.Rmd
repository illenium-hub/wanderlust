...
### Range and defence classfied by result
```{r,echo=FALSE,message=FALSE,dpi=500,warning=FALSE}
...
### Dribble and touch classfied by result
```{r,echo=FALSE,message=FALSE,dpi=500,warning=FALSE}
...
## 4.5 C-tree analysis 
```{r,echo=FALSE,message=FALSE,dpi=500}
library(party)
...
plot(dt)
```
_From the C-tree, we could see the decisive relationship between variables. Defence distance could related to touch time and shooting range. Shooting range which largely influence the other variables and might directly related to game result._   
...
knitr::include_graphics("/Applications/BU/BU/678 Applied Statistical Modeling/MA678 midterm project/678 midterm project shicheng wang/picture/model1.png")
```
_There are 242 players in my dataset, each player has their own constructed function.Picture above shows part of the model summary._  
...
_In this model, the majority of the coefficient are significant. But the according to the binned residual plot, there exists an obvious pattern, and the outliers are pretty large, hence this model needs improvement._  
...
* Model check
I draw the binned plots to check if the model fits well.  
```{r,dpi=500,echo=FALSE}
...
```{r out.width = "50%", fig.align = "center", echo=FALSE , message=FALSE, warning=FALSE}
knitr::include_graphics("/Applications/BU/BU/678 Applied Statistical Modeling/MA678 midterm project/678 midterm project shicheng wang/picture/model2.png")
```
...
* Model check
I use pp-check function to check the distribution of fitting output.  
```{r,echo=FALSE}
...
### check visualization
car::qqPlot(red22)
plot(red22)+abline(h=mean(red22),lwd=4,col="skyblue")
```
_The residual plot shows the residuals are randomly distributed but mean value below 0. The qq-plot indicate the residual do not has more information but the mean still below 0. The model fits not so well in this way._     
...
```{r, warning=FALSE, message=FALSE,dpi=500,include=FALSE}
### m3
...
In order to measure which kind of mutilevel model is best fitted my dataset, the anova test is conducted.  
```{r,echo=FALSE}
anova(m2,m3,m1)
```
